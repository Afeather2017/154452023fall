...
inline auto IsTxnCommited(txn_id_t id) -> bool {
  return (id & TXN_START_ID) == 0;
}

inline auto IsTempTs(timestamp_t ts) -> bool {
  return (ts & TXN_START_ID) != 0;
}

inline auto GetTxnId(txn_id_t id) {
